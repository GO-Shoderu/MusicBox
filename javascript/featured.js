$(document).ready(function(){
  ...
  var rank = $(".movies").find("span.dura");
  var img = $(".movies").find("img");
  var songClip = $(".movies").find("source.audio");


  var page = 'https://cors-anywhere.herokuapp.com/https://api.deezer.com/playlist/908622995';
  ...
  req.onreadystatechange = function()
  {
    if(req.readyState == 4 && req.status == 200){
      ...
      for (var i = 0; i < title.length; i++) {
        ...
        rank[i].innerHTML = values.tracks.data[i].duration + "s";
        songClip[i].setAttribute("src", values.tracks.data[i].preview);
        console.log(values.tracks.data[i].preview);

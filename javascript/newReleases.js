  ...
  req.onreadystatechange = function()
  {
    if(req.readyState == 4 && req.status == 200){
      ...
      for (var i = 0; i < title.length; i++) {
        title[i].innerHTML = values.albums.data[i + 1].title;
        artist[i].innerHTML = values.albums.data[i + 1].artist.name;
        // rank[i].innerHTML = "#" + values.albums.data[i].position + " / " + values.tracks.data.length;
        img[i].setAttribute("src", values.albums.data[i + 1].cover_xl);
